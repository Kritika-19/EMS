...
class BomWizard(models.TransientModel):
    ...
    def import_bom(self):
        ...
        for row in file_data:
            if self.select_file == 'csv' and len(row) != 6:
                raise ValidationError("You can let empty cell in csv file or please use xls file.")
            if row[0] == "" :
                raise ValidationError("Please Assign The Product Name.")
            ...
            mrp_obj_update.update({
                                ...
                                'product_qty': row[2] or "",
                                'product_uom_id': product_tem.uom_id.id or False,
            ...
            line_vals.update({
                            'product_id': product_rows.id or False,
                            'product_qty': row[4] and int(row[4]) or 1,
                            'product_uom_id': product_rows.uom_id.id or False,
                            })
            l2 = [(0, 0, line_vals)]
