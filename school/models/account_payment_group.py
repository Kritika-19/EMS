...
class AccountPaymentGroup(models.Model):
    ...
    def post(self):
        ...
        if 'active_id' in self._context:
            for stud_line in self.payment_ids:
                line = self.env['student.invoice.line'].search(
                    [('name', '=', stud_line.student_id.id), ('account_id', '=', self._context['active_id'])])
                line.write({'current_due_amount': line.current_due_amount - stud_line.amount,
                            'paid_amount': line.paid_amount + stud_line.amount})
...
class AccountMoveLine(models.Model):
    ...
    def _compute_payment_group_matched_amount(self):
        ...
        group, cuanto se pago para la lna en cuestin.
        """
