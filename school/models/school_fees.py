...
class StudentFeesRegister(models.Model):
    ...
    date = fields.Date(string='Date', required=True, help="Date of register",
                       default=lambda *a: time.strftime('%Y-%m-%d'))
    number = fields.Char(string='Number', readonly=True, default=lambda self: _('New'))
    line_ids = fields.One2many('student.payslip', 'register_id', string='PaySlips')
    ...
    state = fields.Selection([('draft', 'Draft'), ('confirm', 'Confirm')], string='State', readonly=True,
                             default='draft')
    journal_id = fields.Many2one('account.journal', string='Journal', help="Select Journal", required=False)
    company_id = fields.Many2one('res.company', string='Company', required=True,
                                 change_default=True, readonly=True,
                                 default=lambda obj_c: obj_c.env['res.users'].
                                 browse([obj_c._uid])[0].company_id)
    fees_structure = fields.Many2one('student.fees.structure', string='Fees Structure')
    standard_id = fields.Many2one('standard.standard', string='Class')
...
class StudentPayslipLine(models.Model):
    ...
    line_ids = fields.One2many('student.payslip.line.line', 'slipline_id', string='Calculations')
    slip_id = fields.Many2one('student.payslip', string='Pay Slip')
...
class StudentPayslip(models.Model):
    ...
    def _compute_invoice(self):
        ...
        for rec in self:
            rec.invoice_count = inv_obj.search_count([('student_payslip_id', '=', rec.id)])

    basic_parent_id = fields.Many2one('school.parent', string='Basic Parent')
    discount_ids = fields.Many2many('ems.discount', 'fee_payslip_discount_rel', 'fee_payslip_id', 'discount_id',
                                    string='Discount')
    fees_structure_id = fields.Many2one('student.fees.structure', string='Fees Structure',
    ...
    medium_id = fields.Many2one('standard.medium', string='Medium')
    register_id = fields.Many2one('student.fees.register', string='Register')
    name = fields.Char(string='Description')
    number = fields.Char(string='Number', readonly=True, default=lambda self: _('New'))
    student_id = fields.Many2one('student.student', string='Student', required=True)
    date = fields.Date(string='Date', readonly=True, help="Current Date of payslip",
                       default=lambda *a: time.strftime('%Y-%m-%d'))
    line_ids = fields.One2many('student.payslip.line', 'slip_id', string='PaySlip Line')
    total = fields.Monetary(string="Total", readonly=True, help="Total Amount")
    ...
    account_id = fields.Many2one('account.account', string="Account")
    tax_ids = fields.Many2many('account.tax', 'taxes_fee_receipt_rel', 'fee_receipt_id', 'fee_account_id', string="Tax")
    invoice_count = fields.Integer(string="# of Invoices", compute="_compute_invoice")
    paid_amount = fields.Monetary(string='Paid Amount', help="Amount Paid")
    due_amount = fields.Monetary(string='Due Amount', help="Amount Remaining")
    currency_id = fields.Many2one('res.currency', string='Currency')
    currency_symbol = fields.Char(related='currency_id.symbol', string='Symbol')
...
class StudentFees(models.Model):
    ...
    def _onchange_name(self):
        if self.name:
            self.student_first_name = self.name + ' ' + self.middle
