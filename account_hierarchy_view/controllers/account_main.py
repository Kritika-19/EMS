...
class CoAReportController(http.Controller):
    ...
    @http.route('/account_hierarchy_view/<string:output_format>/<string:report_name>/<int:report_id>', type='http', auth='user')
    def report(self, output_format, report_name, token, report_id=False, **kw):
        uid = request.session.uid
        domain = [('create_uid', '=', uid)]
        coa = request.env['account.open.chart'].sudo(uid).search(domain, limit=1)
        line_data = json.loads(kw['data'])
        try:
            if output_format == 'pdf':
                response = request.make_response(
                    coa.with_context(active_id=report_id).get_pdf(report_id, line_data),
                    headers=[
...
class ExcelExportView(ExcelExport):
    ...
    def export_xls_view(self, data, token):
        ...
        user_context = request.env['account.open.chart'].browse(wiz_id)._build_contexts()
        lines = request.env['account.open.chart'].with_context(print_mode=True).get_pdf_lines(wiz_id, line_data)
        ...
        for line in lines:
            ...
            type = data[2]
            currency = data[3]
            debit = self.remove_extra_data(data[4], currency_symbol)
            credit = self.remove_extra_data(data[5], currency_symbol)
            balance = self.remove_extra_data(data[6], currency_symbol)
