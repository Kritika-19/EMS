...
class OpenAccountChart(models.TransientModel):
    ...
    def _build_contexts(self):
        self.ensure_one()
        result = {}
        result['state'] = self.target_move or 'posted'
    ...
    def account_chart_open_window(self):
        ...
        if self.env['account.account'].search([('parent_id','!=',False)],limit=1):
            result = self.env.ref('account_hierarchy_view.open_view_account_tree').read([])[0]
        else:
            result = self.env.ref('account_hierarchy_view.open_view_account_noparent_tree').read([])[0]
...
#                             data.get('credit'),
    ...
    @api.model
    def get_pdf_lines(self, wiz_id, line_data=[]):
        lines = self.browse(wiz_id).get_all_lines()
        return lines

    def get_pdf(self, wiz_id, line_data=[]):
        lines = self.with_context(print_mode=True).get_pdf_lines(wiz_id, line_data)
        ...
        body = self.env['ir.ui.view'].render_template(
            "account_hierarchy_view.report_coa_heirarchy_print",
...
#         return self.env['report'].render('account.report_generalledger', docargs)
    ...
    def _get_html(self):
        ...
        result['html'] = self.env.ref('account_hierarchy_view.report_coa_heirarchy').render(rcontext)
        return result
...
#     @api.multi
#     def open_coa_posted(self):
#         wiz_id = self.env['account.open.chart'].create({'target_move': 'posted'})
#         res = wiz_id.account_chart_open_window()
#         return res
