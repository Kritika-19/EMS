...
class BankStatement(models.Model):
    ...
    acc_move_line_ids = fields.One2many('custom.account.move.line', 'statement_id' , string="Account Move Line")
    total_new = fields.Integer(string="Total Amount")
    ...
    @api.model
    def total_amount_in_bank_statement_cron(self):
    ...
    def _update_payment_table(self):
        ...
        for payment in payments:
            ...
            payment.write({'statement_id': self.id,
                           'statement_line_id': line_id.id,
                           'custom_amount': line_id.amount,
                           'move_id': move.id,})
    ...
    @api.multi
    def custom_account_statement_report(self):
        return self.env.ref('sh_payment.action_custom_report_account_statement').report_action(self)
...
class AccountBankStatementLine(models.Model):
    ...
    def process_reconciliation(self, counterpart_aml_dicts=None, payment_aml_rec=None, new_aml_dicts=None):
        ...
        if counterpart_aml_dicts or new_aml_dicts:
            ...
            payment and payment.write({'payment_reference': move.name,'statement_id': move.statement_id.id,
                                       'statement_line_id': self.id,
                                       'custom_amount': self.amount,
                                       'move_id': move.id,})
        elif self.move_name:
...
class AccountMove(models.Model):
    ...
    statement_id = fields.Many2one('account.bank.statement', string="Bank Statement", readonly=True)
    
class account_payment(models.Model):
    _inherit = "account.payment"
    
    statement_id = fields.Many2one('account.bank.statement', string="Bank Statement", readonly=True)
    move_id = fields.Many2one('account.move', string="Account Move Line", readonly=True)
    statement_line_id = fields.Many2one('account.bank.statement.line', string="Bank Statement", readonly=True)
    custom_amount = fields.Monetary(digits=0,)
...
class CustomAccountMoveLine(models.Model):
    ...
    account_payment_id = fields.Many2one('account.payment',string="Account Payent", readonly=True) 
    name = fields.Many2one('account.account',string="Debit A/C No", readonly=True)
    ac_name = fields.Char(related='name.name',string="Debit A/C Name", readonly=True)
    narration = fields.Char("Narration", readonly=True)
    statement_id = fields.Many2one('account.bank.statement', string="Bank Statement", readonly=True)
    amount = fields.Monetary(string="Amount", readonly=True)
    tax = fields.Monetary(string="Tax", readonly=True)
    total = fields.Monetary(string="Total",compute='_compute_total', readonly=True)
    company_id = fields.Many2one('res.company', required=True, index=True, string='Company', default=lambda self: self.env.user.company_id)
